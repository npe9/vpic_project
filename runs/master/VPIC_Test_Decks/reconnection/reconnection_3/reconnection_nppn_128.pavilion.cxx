...
begin_initialization
{
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  double Lx  =      (17.0/16.0)*1.50*30.0/sqrt(mi_me)*di; // size of box in x dimension
  double Ly  = REPLACE_Ly_scale*0.50*15.0/sqrt(mi_me)*di; // size of box in y dimension
  double Lz  =                  1.50*15.0/sqrt(mi_me)*di; // size of box in z dimension

  double topology_x = 8;  // Number of domains in x, y, and z
  double topology_y = 2*REPLACE_topology_y_scale; 
  double topology_z = 8;  

  double nx = 408;
  double ny = 64*REPLACE_ny_scale;
  double nz = 192;
